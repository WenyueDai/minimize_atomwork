...
- [example_antibody_antigen_chunked.yaml](example_antibody_antigen_chunked.yaml)
- [example_vhh_antigen_chunked.yaml](example_vhh_antigen_chunked.yaml)
- [example_protein_protein_chunked.yaml](example_protein_protein_chunked.yaml)
- [HPC_SLURM_GUIDE.md](HPC_SLURM_GUIDE.md)

## Run with internal chunk workers

Activate the conda environment first for real-time progress output:
```bash
...
See [../README.md](README.md) for the field glossary and flag guidance.
For concrete Slurm submission recipes, see [HPC_SLURM_GUIDE.md](large_run/HPC_SLURM_GUIDE.md).

## Step By Step: What `run-chunked` Does

Using [example_antibody_antigen_chunked.yaml](large_run/example_antibody_antigen_chunked.yaml) as the reference.
...
2. Rebuilds `Context` and runs each plugin's `run(ctx)`. Each plugin calls `available(ctx)` first; skips the structure silently if the check fails (e.g. `abepitope_score` when the package is absent, `antibody_cdr_sequences` when no antibody chain is found).
...
- `cdr_entropy` writes position-wise Shannon entropy rows for numbered CDR residues to `dataset.parquet`; if `sequence` is selected it also writes sequence-level summary rows.
...
With `cluster.mode` set and no `interface_side`, chunked examples emit both `left` and `right` jobs automatically.
...
- `cluster` is now explicit opt-in at the parameter level too: keep `"cluster"` in `dataset_analyses`, but also set `dataset_analysis_params.cluster.mode`.
